...
public class MovementDescend extends Movement {
    ...
    public MovementState updateState(MovementState state) {
        ...
        if ((playerFeet.equals(dest) || playerFeet.equals(fakeDest)) && (MovementHelper.isLiquid(ctx, dest) || ctx.mob().position().y - dest.getY() < 0.5)) { // lilypads
        ...
        double x = ctx.mob().position().x - (src.getX() + 0.5);
        double z = ctx.mob().position().z - (src.getZ() + 0.5);
