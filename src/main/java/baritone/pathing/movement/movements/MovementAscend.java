...
public class MovementAscend extends Movement {
    ...
    public MovementState updateState(MovementState state) {
        ...
        double flatDistToNext = xAxis * Math.abs((dest.getX() + 0.5D) - ctx.mob().position().x) + zAxis * Math.abs((dest.getZ() + 0.5D) - ctx.mob().position().z);
        double sideDist = zAxis * Math.abs((dest.getX() + 0.5D) - ctx.mob().position().x) + xAxis * Math.abs((dest.getZ() + 0.5D) - ctx.mob().position().z);

        double lateralMotion = xAxis * ctx.mob().getDeltaMovement().z + zAxis * ctx.mob().getDeltaMovement().x;
