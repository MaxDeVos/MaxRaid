...
public class MovementParkour extends Movement {
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (ctx.playerFeet().equals(dest)) {
            ...
            if (ctx.mob().position().y - ctx.playerFeet().getY() < 0.094) { // lilypads
                state.setStatus(MovementStatus.SUCCESS);
            }
        } else if (!ctx.playerFeet().equals(src)) {
            if (ctx.playerFeet().equals(src.relative(direction)) || ctx.mob().position().y - src.y > 0.0001) {
                if (!MovementHelper.canWalkOn(ctx, dest.below()) && !ctx.mob().isOnGround() && MovementHelper.attemptToPlaceABlock(state, baritone, dest.below(), true, false) == PlaceResult.READY_TO_PLACE) {
                ...
                if (dist == 3 && !ascend) { // this is a 2 block gap, dest = src + direction * 3
                    double xDiff = (src.x + 0.5) - ctx.mob().position().x;
                    double zDiff = (src.z + 0.5) - ctx.mob().position().z;
