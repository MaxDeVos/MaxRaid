...
public class PathExecutor implements IPathExecutor, Helper {
    ...
    public boolean onTick() {
        ...
        if (movementStatus == SUCCESS) {
            ...
            if (!sprintNextTick) {
                ctx.mob().setSprinting(false); // letting go of control doesn't make you stop sprinting actually
            }
            ticksOnCurrent++;
    ...
    private Tuple<Double, BlockPos> closestPathPos(IPath path) {
        ...
        for (IMovement movement : path.movements()) {
            for (BlockPos pos : ((Movement) movement).getValidPositions()) {
                double dist = VecUtils.entityDistanceToCenter(ctx.mob(), pos);
                if (dist < best || best == -1) {
                    best = dist;
                    bestPos = pos;
                }
    ...
    private boolean possiblyOffPath(Tuple<Double, BlockPos> status, double leniency) {
        ...
        if (distanceFromPath > leniency) {
            ...
            if (path.movements().get(pathPosition) instanceof MovementFall) {
                ...
                return VecUtils.entityFlatDistanceToCenter(ctx.mob(), fallDest) >= leniency; // ignore Y by using flat distance
            } else {
                return true;
            }
        } else {
    ...
    public boolean snipsnapifpossible() {
        if (!ctx.mob().isOnGround() && ctx.world().getFluidState(ctx.playerFeet()).isEmpty()) {
            // if we're falling in the air, and not in water, don't splice
            return false;
        } else {
            // we are either onGround or in liquid
            if (ctx.mob().getDeltaMovement().y < -0.1) {
    ...
    private boolean shouldSprintNextTick() {
        ...
        if (current instanceof MovementAscend && pathPosition != 0) {
            ...
            if (prev instanceof MovementDescend && prev.getDirection().above().equals(current.getDirection().below())) {
                ...
                if (ctx.mob().position().y >= center.getY() - 0.07) {
    ...
    private static boolean skipNow(IPlayerContext ctx, IMovement current) {
        double offTarget = Math.abs(current.getDirection().getX() * (current.getSrc().z + 0.5D - ctx.mob().position().z)) + Math.abs(current.getDirection().getZ() * (current.getSrc().x + 0.5D - ctx.mob().position().x));
        if (offTarget > 0.1) {
            return false;
        }
        // we are centered
        ...
        // wait 0.3
        double flatDist = Math.abs(current.getDirection().getX() * (headBonk.getX() + 0.5D - ctx.mob().position().x)) + Math.abs(current.getDirection().getZ() * (headBonk.getZ() + 0.5 - ctx.mob().position().z));
        return flatDist > 0.8;
    }
