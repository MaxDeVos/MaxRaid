...
public final class LookBehavior extends Behavior implements ILookBehavior {
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        ...
        switch (event.getState()) {
            case PRE: {
                if (this.force) {
                    ctx.mob().setYRot(this.target.getYaw());
                    float oldPitch = ctx.mob().getXRot();
                    float desiredPitch = this.target.getPitch();
                    ctx.mob().setXRot(desiredPitch);
                    ctx.mob().setYRot((float) (ctx.mob().getYRot() + (Math.random() - 0.5) * Baritone.settings().randomLooking.value));
                    ctx.mob().setXRot((float) (ctx.mob().getXRot() +  (Math.random() - 0.5) * Baritone.settings().randomLooking.value));
    ...
    private void nudgeToLevel() {
        if (ctx.mob().getXRot() < -20) {
            ctx.mob().setXRot(ctx.mob().getXRot() + 1);
        } else if (ctx.mob().getXRot() > 10) {
            ctx.mob().setXRot(ctx.mob().getXRot() - 1);
        }
    }
}
