...
public final class PathingBehavior extends Behavior implements IPathingBehavior, Helper {
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        if (current != null) {
            //TODO BaritoneSpigot - uh we'll have to handle this separately
    ...
    public BetterBlockPos pathStart() {
        ...
        if (!MovementHelper.canWalkOn(ctx, feet.below())) {
            if (ctx.mob().isOnGround()) {
                double playerX = ctx.mob().position().x;
                double playerZ = ctx.mob().position().z;
            ...
            synchronized (pathPlanLock) {
                ...
                if (talkAboutIt && current != null && current.getPath() != null) {
                    if (goal.isInGoal(current.getPath().getDest())) {
                        logDebug("" +
                                "Finished finding a path from " + start + " to " + goal + ". " + current.getPath().getNumNodesConsidered() + " nodes considered");
                    } else {
