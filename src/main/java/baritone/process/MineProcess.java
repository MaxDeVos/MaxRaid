...
public final class MineProcess extends BaritoneProcessHelper implements IMineProcess {
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        if (desiredQuantity > 0) {
            //TODO mining?
//            int curr = ctx.mob().getInventory().items.stream()
...
//            }
        ...
        if (shaft.isPresent() && ctx.mob().isOnGround()) {
            ...
            for (int y = playerFeet.getY() - searchDist; y <= playerFeet.getY() + searchDist; y++) {
                for (int z = playerFeet.getZ() - searchDist; z <= playerFeet.getZ() + searchDist; z++) {
                    ...
                    if (filter.has(bsi.get0(x, y, z))) {
                        ...
                        if ((Baritone.settings().legitMineIncludeDiagonals.value && knownOreLocations.stream().anyMatch(ore -> ore.distSqr(pos) <= 2 /* sq means this is pytha dist <= sqrt(2) */)) || RotationUtils.reachable(ctx.mob(), pos, fakedBlockReachDistance).isPresent()) {
    ...
    private static List<BlockPos> prune(CalculationContext ctx, List<BlockPos> locs2, BlockOptionalMetaLookup filter, int max, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        List<BlockPos> locs = locs2
                ...
                .sorted(Comparator.comparingDouble(ctx.getBaritone().getPlayerContext().mob().blockPosition()::distSqr))
